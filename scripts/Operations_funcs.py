...
class GetCorrespondingFile:
    ...
    def getJibenxinxi(companys):
        ...
        list_length = len(companys)
        print(list_length)
        iter_size = 1000
        current = 0
        df_all_datas = pd.DataFrame(columns=headers_jibenxinxi)
        # 根据广告主查广告数据,以1000家为查询批次
        while current < list_length:
            end = current + iter_size
            recruit_segment = companys[current:end]
            result_cursor = col.aggregate([
                {'$match': {'entName': {'$in': recruit_segment}}},
                {'$sort': {'storage_time': -1}},
                {'$group': {'_id': "$entName",
                            '企业名称': {'$first': '$entName'},
                            '法定代表人': {'$first': '$frName'},
